...
final class SettingsStore {
    ...
    private func record(_ status: SMAppService.Status) {
        switch status {
        case .notRegistered:
            self.launchAtLoginError = "Clockwork is not registered as a login item."
            self.defaults.set("not registered", forKey: Keys.launchAtLoginStatus)
        case .requiresApproval:
            self.launchAtLoginError = "Allow Clockwork in System Settings → General → Login Items."
        ...
        case .notFound:
            self.launchAtLoginError = "Install Clockwork in Applications, then reopen it."
