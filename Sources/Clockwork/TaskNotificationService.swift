...
final class TaskNotificationService: NSObject, UNUserNotificationCenterDelegate {
    ...
    private let logger = Logger(subsystem: "is.ian.clockwork", category: "Notifications")
    ...
    private func deliver(_ event: TaskRunEvent, taskID: UUID, runID: String) async -> DeliveryOutcome {
        ...
        guard authorization == .allowed else {
            if authorization == .denied {
                self.logger.notice("Notifications are disabled for Clockwork")
            }
    ...
    {
        Logger(subsystem: "is.ian.clockwork", category: "Notifications")
            .info("Presenting attention notification while Clockwork is active")
        return [.banner, .sound]
    }
