...
do {
    ...
    FileHandle.standardError.write(Data("scheduler: \(error.localizedDescription)\n".utf8))
    exit(1)
}
...
func run() throws {
    ...
    case "add":
        ...
        guard !commands.isEmpty else { throw SchedulerError.invalidTask("Add at least one --command.") }
    ...
    default:
        throw SchedulerError.invalidTask("Unknown command '\(command)'. Run scheduler help.")
    }
}
...
func requireTask(_ identifier: String?) throws -> ScheduledTask {
    ...
    else { throw SchedulerError.taskNotFound("No task found for '\(value)'.") }
    return task
}

func required<T>(_ value: T?, message: String) throws -> T {
    guard let value else { throw SchedulerError.invalidTask(message) }
    return value
}
...
func parseSchedule(arguments: inout CLIArguments) throws -> TaskSchedule {
    if let every = arguments.option("--every") {
        guard let suffix = every.last, let value = Int(every.dropLast()), value > 0 else {
            throw SchedulerError.invalidTask("Use --every 30m or --every 6h.")
        }
        switch suffix {
        case "m": return .interval(value: value, unit: .minutes)
        case "h": return .interval(value: value, unit: .hours)
        default: throw SchedulerError.invalidTask("Use --every 30m or --every 6h.")
        }
    }
    ...
    if let weekly = arguments.option("--weekly") {
        ...
        guard parts.count == 2 else { throw SchedulerError.invalidTask("Use --weekly mon,wed@09:00.") }
    ...
    throw SchedulerError.invalidTask("Choose one schedule: --every 6h, --daily 09:00, or --weekly mon,fri@09:00.")
}
...
func parseTime(_ value: String) throws -> (hour: Int, minute: Int) {
    ...
    guard parts.count == 2, (0...23).contains(parts[0]), (0...59).contains(parts[1]) else {
        throw SchedulerError.invalidTask("Time must use 24-hour HH:MM format.")
    }
...
func parseWeekday(_ value: String) throws -> Weekday {
    guard let day = Weekday.allCases.first(where: { $0.shortName.lowercased().hasPrefix(value.lowercased()) }) else {
        throw SchedulerError.invalidTask("Unknown weekday '\(value)'.")
    }
    return day
}
...
func explainCron(_ expression: String) throws -> String {
    ...
    guard fields.count == 5 else { throw SchedulerError.invalidTask("A cron expression needs five fields.") }
    ...
    return "Valid-looking cron, but Scheduler cannot translate this pattern yet. " +
...
func printHelp() {
    print("""
    Scheduler, human-friendly recurring commands

    scheduler list [--json]
    scheduler show <id|name> [--json]
    scheduler runs <id|name> [--json]
    scheduler add --name NAME --command CMD [--command CMD] [--cwd PATH] --every 6h|--daily 09:00|--weekly mon,fri@09:00
    scheduler update <id|name> [--name NAME] [--command CMD] [--cwd PATH|-] [schedule] [--enable|--disable]
    scheduler enable|disable|delete|run <id|name> [--json]
    scheduler attention --title TITLE --message MESSAGE [--next-step TEXT]
    scheduler sync
