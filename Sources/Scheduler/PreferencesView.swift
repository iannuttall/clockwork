...
private struct GeneralPreferencesPane: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 22) {
            PaneTitle(title: "General", subtitle: "Keep Scheduler available without keeping it in your Dock.")
            Card {
                Toggle("Start Scheduler automatically at login", isOn: self.$settings.launchAtLogin)
...
private struct CommandLinePane: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            Card {
                Text("Install the `scheduler` command")
                    .font(.headline)
                Text("Installs to ~/.local/bin/scheduler. It supports JSON output and full task management.")
                    .font(.callout)
                    .foregroundStyle(.secondary)
                HStack {
                ...
                Text([
                    "scheduler list --json",
                    "scheduler add --name 'Update Codex' --command 'codex update' --every 6h",
    ...
    private func installCLI() {
        do {
            ...
            let source = executable.deletingLastPathComponent().appendingPathComponent("schedulercli")
            ...
            let destination = directory.appendingPathComponent("scheduler")
            try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
            try? FileManager.default.removeItem(at: destination)
            try FileManager.default.copyItem(at: source, to: destination)
            try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: destination.path)
            self.installMessage = "Installed to ~/.local/bin/scheduler"
        } catch {
...
private enum CLIInstallError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingExecutable: "Could not locate the Scheduler app."
        case .missingCLI: "This build does not contain the Scheduler CLI."
        }
    }
}
