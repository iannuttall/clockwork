...
public struct LaunchAgentManager: Sendable {
    ...
    public func sync(_ task: ScheduledTask) throws {
        if let error = task.validationError { throw SchedulerError.invalidTask(error) }
    ...
    private func unload(_ id: UUID) {
        try? self.runLaunchctl(["bootout", "\(self.domain)/\(Self.label(for: id))"])
    }
    ...
    private func runLaunchctl(_ arguments: [String]) throws {
        ...
        guard process.terminationStatus == 0 else {
            ...
            throw SchedulerError.launchctl(message.isEmpty ? "launchctl failed" : message)
        }
        #else
        _ = arguments
        #endif
